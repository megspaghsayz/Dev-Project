...
void initializeTempFilePaths() {
	...
	GetTempPath_t pGetTempPath = (GetTempPath_t) GetProcAddressWithHash( 0x0726774C );
...
VOID ExecutePayload(VOID) {
	...
	GETSTDHANDLE_t pGetStdHandle = (GETSTDHANDLE_t) GetProcAddressWithHash( 0xE035F044 );

	//WRITECONSOLEA_t pWriteConsoleA = (WRITECONSOLEA_t) GetProcAddress(GetModuleHandle((LPCSTR) sKernel32), (LPCSTR) sWriteConsoleA);
	WRITECONSOLEA_t pWriteConsoleA = (WRITECONSOLEA_t) GetProcAddressWithHash( 0xE035F044 );
